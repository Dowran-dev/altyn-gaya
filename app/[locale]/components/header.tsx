...
export default function Header() {
  ...
  const navItems = [
    ...
    { name: t("header.products"), href: "/shop" },
    // { name: t("header.news"), href: "/news-page" },
    { name: t("header.faq"), href: "/faq" },
    { name: t("header.contacts"), href: "/contacts-page" },
  ];
  ...
  const changeLanguage = (lang: string) => {
    ...
    // Remove empty string and current locale (e.g. ['', 'en', 'shop'] -> ['shop'])
          ...
          <div className="flex items-center justify-between h-16 md:h-20">
            ...
            <nav className="hidden lg:flex items-center space-x-6">
              {navItems.map((item) => (
                <div
                  key={item.name}
                  className="relative"
                  onClick={() => item.name === t("header.products") ? setIsProductsDropdownOpen(prev => !prev) : null}
                >
                  {item.name === t("header.products") ? (
                    <>
                      <div className="flex items-center text-gray-700 dark:text-gray-300 hover:text-[#fb4b06] dark:hover:text-[#fb4b06] transition-colors cursor-pointer">
                        <span>{item.name}</span>
                        <ChevronDown className={`ml-1 w-4 h-4 transition-transform ${isProductsDropdownOpen ? "rotate-180" : ""}`} />
            ...
            <div className="flex items-center space-x-1 md:space-x-3">
              ...
              <div className="relative" ref={languageDropdownRef}>
                <button
                  className="p-2 text-gray-700 dark:text-gray-300 hover:text-[#fb4b06] dark:hover:text-[#fb4b06] transition-colors flex items-center"
                  onClick={() => setIsLanguageDropdownOpen(!isLanguageDropdownOpen)}
                  aria-label="Change language"
                >
                  <Globe className="w-5 h-5" />
                  <span className=" text-xs hidden md:inline-block">{getCurrentLanguage().flag}</span>
                </button>
                
                {isLanguageDropdownOpen && (
                  <div className="absolute top-full right-0 mt-1 bg-white dark:bg-gray-800 shadow-xl rounded-xl overflow-hidden w-36 z-20 border dark:border-gray-700">
                    <div className="py-1">
                      {languages.map((lang) => (
                        <button
                          key={lang.code}
                          className={`w-full text-left px-4 py-2 flex items-center ${
                            locale === lang.code 
                              ? "text-[#fb4b06] bg-orange-50 dark:bg-gray-700" 
                              : "text-gray-700 dark:text-gray-300 hover:bg-orange-50 dark:hover:bg-gray-700 hover:text-[#fb4b06]"
                          } transition-colors`}
                          onClick={() => {
                  ...
                  {languages.map((lang) => (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-full text-sm border ${
                        locale === lang.code
                          ? "bg-[#fb4b06]/10 border-[#fb4b06]/30 text-[#fb4b06]"
