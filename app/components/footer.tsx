...
async function fetchProductCategories() {
  const res = await fetch("http://localhost:3000/api/products", {
    cache: "no-store", // Ensures fresh data on each request; adjust caching as needed
  });
  const data = await res.json();
  return data; // Returns the array of ProductCategories
}

export default async function Footer() {
  // Fetch the product categories from the API
  const productsData = await fetchProductCategories();

  // Extract category names from the productsData and limit to 4
  const categories = productsData.map((item) => item.category).slice(0, 4);
      ...
      <footer className="bg-[#0a0d42] py-12 text-white relative overflow-hidden">
        <div className="container mx-auto px-4 relative z-10">
          <div className="grid md:grid-cols-4 gap-8">
            ...
            {/* Dynamically populated Product Categories with max 4 limit */}
            <div>
              <h4 className="text-xl font-bold mb-6">Продукция</h4>
              <ul className="space-y-3">
          ...
          <div className="mt-12 pt-8 border-t border-white/10 flex flex-col md:flex-row justify-between items-center">
            ...
            <div className="flex space-x-6">
              <a
                href="#"
                className="text-white/50 hover:text-[#fb4b06] transition-colors"
              >
