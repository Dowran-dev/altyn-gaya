...
//   // Calculate card position for 3D carousel effect
//   const getCardPosition = () => {
//     // Active card is centered
//     if (isActive) return "opacity-100 z-20 transform-none";
...
//     if (adjustedDistance === 1 || adjustedDistance === -1) {
//       // Adjacent cards
//       const translateX = adjustedDistance * 10;
//       const translateZ = -100;
//       const opacity = 0.7;
//       const zIndex = 10;
//       const scale = 0.85;
//       return `opacity-${opacity} z-${zIndex} scale-${scale} translate-x-${translateX} translate-z-${translateZ}`;
//     } else {
//       // Far cards are hidden
//       return "opacity-0 z-0 scale-75";
//     }
//   };
...
//   // Product data array with enhanced features
//   const products = [
//     {
...
//         color: "#1E22AA",
//         features: [
//             "Безопасный состав, не вызывающий раздражения",
//             "Эффективное удаление загрязнений даже в холодной воде",
//             "Экономичный расход благодаря высокой концентрации"
//         ]
//     },
//     {
//         title: "Кондиционер и ополаскиватель для одежды",
//         description: "Придаёт белью мягкость и свежий аромат. Защищает ткань и облегчает глажение.",
//         image: "/images/RealPlus_SuwukSoda_5l_Supreme+.png",
//         color: "#fb4b06",
//         features: [
//             "Нейтрализует статическое электричество",
//             "Делает ткань более мягкой и приятной на ощупь",
//             "Освежающий и стойкий аромат"
//         ]
//     },
//     {
//         title: "Ручной порошок",
//         description: "Мощное средство для ручной стирки. Легко справляется с пятнами и придаёт одежде свежесть.",
//         image: "/images/RealPlus_Soda.png",
//         color: "#fb4b06",
//         features: [
//             "Эффективно удаляет пятна и загрязнения",
//             "Не повреждает структуру ткани",
//             "Легко смывается, не оставляя разводов"
//         ]
//     },
//     {
//         title: "Спрей для одежды и тканей",
//         description: "Свежесть в каждом нажатии. Идеально подходит для одежды, мебели и других трудно стираемых вещей.",
//         image: "/images/YumsaPlus_Sprey_Lilya.jpeg",
//         color: "#fb4b06",
//         features: [
//             "Быстро устраняет неприятные запахи",
//             "Придаёт лёгкий и приятный аромат",
//             "Антибактериальный эффект для защиты тканей"
//         ]
//     }
//   ];
...
// const ProductCard: React.FC<ProductCardProps> = ({ product, isActive, index, activeIndex }) => {
//   // Calculate card position for enhanced 3D carousel effect
//   const getCardPosition = () => {
//     // Active card is centered
//     if (isActive) return "opacity-100 z-20 transform-none";
...
//     // Handle wraparound for continuous rotation effect
//     const adjustedDistance = 
//       distance > 2 ? distance - 5 : 
//       distance < -2 ? distance + 5 : 
//       distance;
...
//       // Card to the right
//       return "opacity-70 z-10 scale-85 translate-x-20 -translate-y-6 rotate-3 translate-z-100";
//     } else if (adjustedDistance === -1) {
//       // Card to the left
//       return "opacity-70 z-10 scale-85 -translate-x-20 -translate-y-6 -rotate-3 translate-z-100";
//     } else if (Math.abs(adjustedDistance) === 2) {
//       // Cards further away
//       const side = adjustedDistance > 0 ? 1 : -1;
//       return `opacity-40 z-0 scale-75 translate-x-${side * 40} -translate-y-12 ${side > 0 ? 'rotate-6' : '-rotate-6'} translate-z-200`;
//     } else {
//       // Far cards are hidden behind
//       return "opacity-0 z-0 scale-60";
//     }
//   };

//   // Enhanced card positioning with proper rotation and z-axis movement
//   const cardPositionClass = isActive ? 
//     "opacity-100 z-20 transform-none" : 
//     "opacity-0 z-0 scale-95 translate-y-8";

//   return (
//     <div className={`absolute inset-0 transition-all duration-700 ease-out ${cardPositionClass}`}>
//       <div className="bg-white rounded-3xl overflow-hidden shadow-2xl mx-auto h-full border border-gray-100 hover:shadow-[0_20px_50px_rgba(30,34,170,0.15)] transition-all duration-500">
//         <div className="flex flex-col md:flex-row h-full">
//           <div className="md:w-3/5 relative overflow-hidden group">
//             {/* Enhanced gradient background with more dynamic feel */}
//             <div 
//               className="h-64 md:h-full w-full relative flex items-center justify-center overflow-hidden"
//               style={{ 
//                 background: `radial-gradient(circle at 30% 70%, ${product.color} 0%, ${product.color}AA 60%, ${product.color}66 100%)` 
//               }}
//             >
//               {/* Glass morphism effect for product image */}
//               <div className="relative z-10 transform transition-all duration-700 group-hover:scale-110 group-hover:rotate-2 perspective-1000">
//                 <div className="absolute inset-0 bg-white/15 rounded-full filter blur-xl transform scale-75 group-hover:scale-100 transition-transform duration-700"></div>
//                 <Image
//                   src={product.image}
//                   alt={product.title}
//                   width={400}
//                   height={400}
//                   className="w-4/5 h-4/5 object-contain p-2 drop-shadow-lg"
//                 />
                
//                 {/* Highlight effect on hover */}
//                 <div className="absolute inset-0 bg-white/10 opacity-0 group-hover:opacity-100 transition-opacity duration-700 rounded-full"></div>
//               </div>
              
//               {/* Enhanced animated decorative elements with more dynamic movement */}
//               <div className="absolute top-0 left-0 w-full h-full overflow-hidden">
//                 {/* Abstract shapes with parallax effect */}
//                 <div className="absolute top-10 left-10 w-24 h-24 rounded-full bg-white/10 animate-pulse-slow"></div>
//                 <div className="absolute bottom-10 right-10 w-20 h-20 rounded-full bg-white/10 animate-pulse-slow" style={{ animationDelay: '0.5s' }}></div>
//                 <div className="absolute top-1/3 right-1/4 w-16 h-16 rounded-full bg-white/10 animate-pulse-slow" style={{ animationDelay: '1s' }}></div>
//                 <div className="absolute -bottom-10 -left-10 w-48 h-48 rounded-full bg-white/5 animate-pulse-slow" style={{ animationDelay: '1.5s' }}></div>
                
//                 {/* Enhanced floating particles with varying sizes and speeds */}
//                 <div className="absolute w-3 h-3 rounded-full bg-white/30 animate-float-complex" style={{ top: '20%', left: '30%' }}></div>
//                 <div className="absolute w-2 h-2 rounded-full bg-white/30 animate-float-complex" style={{ top: '70%', left: '20%', animationDelay: '1s' }}></div>
//                 <div className="absolute w-4 h-4 rounded-full bg-white/30 animate-float-complex" style={{ top: '40%', left: '80%', animationDelay: '2s' }}></div>
//                 <div className="absolute w-6 h-6 rounded-full bg-white/20 animate-float-complex" style={{ top: '85%', left: '50%', animationDelay: '3s' }}></div>
//                 <div className="absolute w-5 h-5 rounded-full bg-white/20 animate-float-complex" style={{ top: '15%', left: '65%', animationDelay: '4s' }}></div>
//               </div>
              
//               {/* Subtle movement pattern in background */}
//               <div className="absolute inset-0 bg-gradient-to-br from-black/5 to-transparent opacity-30"></div>
//             </div>
//           </div>
//           <div className="md:w-1/2 p-8 md:p-10 flex flex-col justify-center">
//             {/* Enhanced category tag with animated gradient */}
//             <div className="inline-flex items-center bg-gradient-to-r from-[#fb4b06]/15 to-[#fb4b06]/5 rounded-full px-4 py-1.5 mb-4 hover:from-[#fb4b06]/20 hover:to-[#fb4b06]/10 transition-all duration-300">
//               <span className="text-[#fb4b06] text-sm py-2 font-semibold tracking-wide">Продукция Altyn Gaya</span>
//             </div>
            
//             <h3 className="text-[#1E22AA] text-2xl md:text-3xl font-bold mb-4 leading-tight" style={{ fontFamily: '"Avenir Next Heavy", system-ui, sans-serif' }}>
//               {product.title}
//             </h3>
//             <p className="text-gray-600 text-base md:text-lg mb-6 md:mb-8 leading-relaxed">
//               {product.description}
...
//             {/* Enhanced feature list with improved animations */}
//             <ul className="mb-6 md:mb-8 space-y-3 md:space-y-4">
//               {product.features && product.features.map((feature, index) => (
//                 <li key={index} className="flex items-start group">
//                   <div className="bg-[#fb4b06]/10 p-2 rounded-full mr-3 group-hover:bg-[#fb4b06] transition-colors duration-300 mt-0.5 transform group-hover:scale-110 transition-transform">
//                     <Check className="text-[#fb4b06] w-4 h-4 group-hover:text-white transition-colors duration-300" />
//                   </div>
...
//                 <>
//                   <li className="flex items-start group">
//                     <div className="bg-[#fb4b06]/10 p-2 rounded-full mr-3 group-hover:bg-[#fb4b06] transition-all duration-300 mt-0.5 transform group-hover:scale-110">
//                       <Check className="text-[#fb4b06] w-4 h-4 group-hover:text-white transition-colors duration-300" />
//                     </div>
//                     <span className="text-gray-700 group-hover:text-gray-900 transition-colors duration-300">Высокое качество</span>
//                   </li>
//                   <li className="flex items-start group">
//                     <div className="bg-[#fb4b06]/10 p-2 rounded-full mr-3 group-hover:bg-[#fb4b06] transition-all duration-300 mt-0.5 transform group-hover:scale-110">
//                       <Check className="text-[#fb4b06] w-4 h-4 group-hover:text-white transition-colors duration-300" />
//                     </div>
//                     <span className="text-gray-700 group-hover:text-gray-900 transition-colors duration-300">Экологичные материалы</span>
//                   </li>
//                   <li className="flex items-start group">
//                     <div className="bg-[#fb4b06]/10 p-2 rounded-full mr-3 group-hover:bg-[#fb4b06] transition-all duration-300 mt-0.5 transform group-hover:scale-110">
//                       <Check className="text-[#fb4b06] w-4 h-4 group-hover:text-white transition-colors duration-300" />
//                     </div>
//                     <span className="text-gray-700 group-hover:text-gray-900 transition-colors duration-300">Оптимальная цена</span>
//                   </li>
//                 </>
//               )}
//             </ul>
            
//             {/* Enhanced button with improved gradient and animation */}
//             <Button className="bg-[#1E22AA] hover:bg-[#1E22AA]/90 rounded-full text-white px-6 py-2 md:px-8 md:py-6 text-sm md:text-md font-semibold group overflow-hidden relative w-full md:w-auto">
//               <span className="relative z-10 flex items-center justify-center gap-2">
//                 Подробнее
//                 <ArrowRight className="w-4 h-4 group-hover:translate-x-2 transition-transform duration-500" />
//               </span>
//               <span className="absolute inset-0 bg-gradient-to-r from-[#fb4b06] via-[#fb4b06]/90 to-[#E85D24] transform scale-x-0 group-hover:scale-x-100 origin-left transition-transform duration-500"></span>
//             </Button>
//           </div>
//         </div>
//       </div>
...
//   // Enhanced slider navigation handlers with transition state
//   const nextProduct = () => {
...
//   // Enhanced touch event handlers for mobile swipe with improved sensitivity
//   const handleTouchStart = (e: React.TouchEvent) => {
//     setIsPaused(true);
//     setTouchStartX(e.touches[0].clientX);
//   };
...
//     // Detect swipe with improved threshold
//     if (diffX > 40) {
//       prevProduct();
//     } else if (diffX < -40) {
//       nextProduct();
...
//     return () => clearInterval(interval);
//   }, [activeProductIndex, isPaused, isTransitioning]);
...
//       <div className="absolute inset-0 overflow-hidden pointer-events-none">
//         <div className="absolute top-1/4 left-1/4 w-40 md:w-80 h-40 md:h-80 rounded-full bg-[#1E22AA]/5 filter blur-2xl md:blur-3xl animate-pulse-slow" style={{ animationDuration: '15s' }}></div>
//         <div className="absolute bottom-1/4 right-1/4 w-48 md:w-96 h-48 md:h-96 rounded-full bg-[#fb4b06]/5 filter blur-2xl md:blur-3xl animate-pulse-slow" style={{ animationDuration: '20s', animationDelay: '2s' }}></div>
//         <div className="absolute top-2/3 left-1/2 w-32 md:w-64 h-32 md:h-64 rounded-full bg-[#E85D24]/5 filter blur-2xl md:blur-3xl animate-pulse-slow" style={{ animationDuration: '18s', animationDelay: '1s' }}></div>
        
//         {/* Additional decorative elements */}
//         <div className="absolute top-10 right-10 w-20 h-20 md:w-40 md:h-40 rounded-full border border-[#1E22AA]/10 opacity-50"></div>
//         <div className="absolute bottom-20 left-20 w-24 h-24 md:w-48 md:h-48 rounded-full border border-[#fb4b06]/10 opacity-50"></div>
//       </div>
      
//       <div className="container mx-auto px-4 relative z-10">
//         <div className="text-center max-w-4xl mx-auto mb-16 md:mb-24">
//           <FadeInView>
//             <div className="inline-block mb-5 bg-gradient-to-r from-[#fb4b06]/20 to-[#fb4b06]/10 rounded-full px-5 md:px-7 py-2.5 transform hover:scale-105 transition-transform duration-300">
//               <span className="text-[#fb4b06] text-xs md:text-sm font-medium uppercase tracking-wider">Наша продукция</span>
//             </div>
//             <h2 className="text-3xl md:text-5xl lg:text-6xl font-bold mb-6 md:mb-8 leading-tight" style={{ fontFamily: '"Avenir Next Heavy", system-ui, sans-serif' }}>
//               <span className="bg-clip-text text-transparent bg-gradient-to-r from-[#1E22AA] to-[#3538C8]">КАЧЕСТВО, КОТОРОЕ </span>
//               <span className="bg-clip-text text-transparent bg-gradient-to-r from-[#fb4b06] to-[#E85D24]">ЗАБОТИТСЯ О ВАС</span>
//             </h2>
...
//         <div 
//           className="relative max-w-6xl mx-auto h-[520px] md:h-[600px] lg:h-[640px]"
//           onMouseEnter={() => setIsPaused(true)}
//           onMouseLeave={() => setIsPaused(false)}
//           onTouchStart={handleTouchStart}
...
//           {/* Enhanced slider navigation with improved animations */}
//           <div className="flex justify-between items-center mt-10 md:mt-16">
//             <Button 
//               onClick={prevProduct} 
//               disabled={isTransitioning}
//               className="w-12 h-12 md:w-16 md:h-16 rounded-full bg-white shadow-xl border border-gray-100 hover:bg-gradient-to-r from-[#fb4b06] to-[#E85D24] hover:border-transparent hover:text-white transition-all duration-300 flex items-center justify-center group disabled:opacity-50"
//               variant="outline"
//               aria-label="Previous product"
//             >
...
//                 <button 
//                   key={index}
//                   onClick={() => !isTransitioning && setActiveProductIndex(index)}
//                   className={`h-3 md:h-4 rounded-full transition-all duration-500 ${
//                     index === activeProductIndex 
//                       ? "bg-gradient-to-r from-[#fb4b06] to-[#E85D24] w-10 md:w-12" 
//                       : "bg-gray-300 hover:bg-gray-400 w-3 md:w-4 hover:w-6"
//                   }`}
//                   aria-label={`Go to slide ${index + 1}`}
//                   aria-current={index === activeProductIndex ? "true" : "false"}
//                   disabled={isTransitioning}
...
//       {/* Add enhanced keyframe animations to stylesheet */}
//       <style jsx global>{`
//         @keyframes float-complex {
//           0%, 100% { transform: translateY(0) translateX(0) rotate(0); }
//           20% { transform: translateY(-15px) translateX(10px) rotate(5deg); }
//           40% { transform: translateY(5px) translateX(15px) rotate(10deg); }
//           60% { transform: translateY(15px) translateX(5px) rotate(5deg); }
//           80% { transform: translateY(5px) translateX(-10px) rotate(-5deg); }
//         }
